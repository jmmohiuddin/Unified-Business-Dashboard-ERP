...
```bash
npm test           # 229 checks: metrics, writes, end-to-end, security
```
...
| **24 routes** | Dashboard, businesses, receivables, purchases, rentals, cheques, services, salon, inventory, CRM, compliance, VAT201, P&L, gratuity, assistant, inbox, security settings, public API |
...
| **AI assistant** | Real Claude tool-calling over the metric layer — no SQL access, evidence links on every answer |
...
| **Tests** | 26 metric · 35 write-layer · 100 end-to-end · 68 security = **229 checks**, all passing |
...
npm run test           # 201 checks across four suites
...
- The AI assistant answers one question at a time. Conversation threading and
  streaming are next; the persistence tables already exist.
