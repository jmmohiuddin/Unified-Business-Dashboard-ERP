...
/**
 * Payroll, imported from the service modules directly rather than through the
 * `@nexus/core` barrel.
 *
 ...
 * features land in one working tree, so the `export * from "./payroll.ts"` line
 * is not there yet. Second — and this survives that being fixed — the barrel
 * drags every service into this package's typecheck, and `@nexus/db` compiles
 * under stricter settings than `@nexus/core` does, so the barrel import turns
 * `npm run typecheck -w @nexus/db` red on files this package never calls.
 * Import the two modules that are actually used.
 */
import type { Principal } from "../../../core/src/rbac.ts";
import type { ServiceContext } from "../../../core/src/services/context.ts";
import {
  commitPayrollRun,
  markPayrollRunPaid,
} from "../../../core/src/services/payroll.ts";
...
async function main() {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (isPast && !noShow) {
        ...
        commissionEntries.push({
          ...
          /**
           * Unpaid on creation, always.
           *
           * This used to be `endsAt < addDays(TODAY, -35)` — a date rule that
           * marked commission "paid" with nothing having paid it. The entries
           * were seed-only, no code path ever read them, and the ledger carried
           * no commission expense at all despite AED 53k of it being earned.
           *
           * A commission entry is now settled by the payroll run that claims
           * it (`services/payroll.ts`, rule 4), which sets `payslip_id` and
           * flips this flag. Pre-marking them here would hide them from that
           * selection and reproduce the same gap the other way round: entries
           * flagged paid that no payslip and no journal can account for.
           */
          earnedOn: isoDate(endsAt), isPaid: false,
        });
      }
    }
  }
  ...
  // ── Expenses, gratuity accrual, drawings ──────────────────────────────────
  // Payroll moved out of this loop; see the FR-C06 section further down.
  console.log("· Operating expenses, gratuity accrual and owner drawings…");
  const monthsBack = 7;
  ...
  for (let m = monthsBack - 1; m >= 0; m--) {
    ...
    /**
     * NO PAYROLL JOURNAL HERE. See the payroll section after the balance
     * trigger is re-enabled.
     *
     * This loop used to post `DR SALARY <whole package> / CR BANK` with
     * `source_table = 'payroll_runs'` and a `source_id` generated by `id()` —
     * pointing at a `payroll_runs` row that was never created, because nothing
     * in the product wrote that table. Seven dangling journals, no payslips, no
     * commission expense, no salaries payable, and a WPS export that derived
     * itself from `employees` because there was nothing else to derive it from.
     *
     * The runs are now generated by `commitPayrollRun`, so the demo ledger
     * contains what the product would actually produce rather than an
     * approximation of it.
     */
  ...
  /* ── Payroll runs — FR-C06 ───────────────────────────────────────────────
   *
   * Generated by the service, never hand-written.
   *
   * Deliberately AFTER the balance trigger is re-enabled: the bulk load
   * disables it for speed, and these are the one set of journals in the seed
   * that the database itself checks leg by leg as they are written. A fixture
   * that a run posts and a run does not is worthless — the point of seeding
   * through `commitPayrollRun` is that the demo data and the product cannot
   * diverge, and the SIF control-total assertion in the e2e suite is precisely
   * the check that would notice if they did.
   *
   * WHICH MONTHS, AND WHY THE LAST ONE IS DIFFERENT.
   *
   * Wages are due on the first day of the following month and this group's bank
   * run goes out on the 3rd — the same seven cash-outflow dates this section
   * replaces. So each completed month is committed on its own last day and paid
   * on the 3rd of the next one.
   *
   * The CURRENT month is committed and deliberately left `approved`, unpaid.
   * That is the state a real employer is in mid-month, it is what makes
   * Salaries Payable a live number on the balance sheet instead of an account
   * that only ever nets to zero, and it is what the WPS export serialises —
   * the file is an instruction to move money that has not moved yet.
   *
   * Its journal is dated the last day of the month, which is after the demo's
   * own "today" of 2026-08-06. That is a real consequence of anchoring the demo
   * mid-month and it is stated rather than hidden: the alternative — no run for
   * the current month — leaves the WPS export with nothing to serialise, which
   * is the incoherence this section exists to remove.
   */
  console.log("· Payroll runs (FR-C06) — one per seeded month, via the service…");

  const payrollPrincipal: Principal = {
    userId: OWNER_USER,
    tenantId,
    ...
    locationIds: null,
    permissions: new Set(["payroll:read", "payroll:approve", "payroll:pay"]),
    isPlatformAdmin: false,
  };

  /** `YYYY-MM` for the eight months ending with the demo's current one. */
  const payrollMonths = Array.from({ length: 8 }, (_, i) => {
    const d = new Date(Date.UTC(TODAY.getUTCFullYear(), TODAY.getUTCMonth() - (7 - i), 1));
    return isoDate(d).slice(0, 7);
  });
  const currentMonth = payrollMonths.at(-1)!;

  /** The 3rd of the month after `period` — when the bank run actually goes. */
  const payDayAfter = (period: string) => {
    const [y, m] = period.split("-").map((x) => parseInt(x, 10));
    return isoDate(new Date(Date.UTC(y!, m!, 3)));
  };

  const payrollCtx = (tx: any): ServiceContext => ({
    tx,
    tenantId,
  ...
  const payrollLog: string[] = [];
  for (const period of payrollMonths) {
    const run = await db.transaction(async (tx) => {
      // Harmless under the seeding role, which bypasses RLS, and correct if it
      // ever stops doing so.
      await tx.execute(sql`select set_config('app.tenant_id', ${tenantId}, true)`);
      return commitPayrollRun(payrollCtx(tx), { period });
    });

    let paidOn: string | null = null;
    if (period !== currentMonth) {
      paidOn = payDayAfter(period);
      await db.transaction(async (tx) => {
        await tx.execute(sql`select set_config('app.tenant_id', ${tenantId}, true)`);
        return markPayrollRunPaid(payrollCtx(tx), { runId: run.runId, paidOn, paidVia: "wps" });
      });
    }
    payrollLog.push(
      `   ${period}  ${String(run.totals.employees).padStart(2)} staff  ` +
        `gross ${run.totals.gross.toFixed(2).padStart(10)}  ` +
        `commission ${run.totals.commission.toFixed(2).padStart(8)}  ` +
        `net ${run.totals.net.toFixed(2).padStart(10)}  ` +
        (paidOn ? `paid ${paidOn}` : "approved, unpaid"),
    );
  }
  for (const line of payrollLog) console.log(line);
